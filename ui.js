...
function refreshBubble() {
  ...
  if (selected.kind === 'build') {
    ...
    inner = `<h3>${def.emoji} ${def.name}${b.lvl > 1 ? ' ★' : ''}${b.halt ? ' · halted' : ''}${!b.built && !b.ruined ? ' · rising' : ''}${b.ruined ? ' · rubble' : ''}</h3>` +
      `<div class="sdesc">${def.desc}</div>`;
    if (b.ruined) {
      const half = Object.entries(costOf(b.type)).map(([k, v]) => `${RES_ICON[k]}${Math.ceil(v * 0.5)}`).join(' ');
      inner += `<div class="sdesc" style="color:var(--bad)">A smoulder of beams. In a few days the forest takes it — or raise it again now, on the same bones.</div>` +
        `<div class="srow"><button class="btn" id="rebuildbtn">Raise it again (${half})</button></div>`;
      wire = () => { $('rebuildbtn').onclick = () => rebuildRuin(b); };
    } else if (!b.built) {
      ...
      if (b.type === 'torch') {
        const f2 = b.fuel || 0;
        const lbl = f2 > 20 ? 'burning strong' : f2 > 8 ? 'burning steady' : f2 > 0 ? 'wants wood, badly' : 'cold and dark — its circle belongs to the night';
        inner += `<div class="srow"><span>The flame</span><b>${lbl}</b></div>`;
      }
      if (b.type === 'garrison') {
        inner += `<div class="srow"><span>Trained as</span><b>${b.troop === 'scout' ? 'scouts — sparks in the dark' : 'archers — death at a distance'}</b></div>`;
      }
      if (b.hp != null && b.hp < B_HP) {
        inner += `<div class="srow"><span>Battered</span><b>${Math.ceil(b.hp)}/${B_HP} — hands will mend it</b></div>`;
      }
      const btns = [];
      if (b.type === 'torch') btns.push(`<button class="btn" id="feedtorchbtn" ${G.res.wood < 5 || (b.fuel || 0) > 26 ? 'disabled' : ''}>Feed it (🪵5)</button>`);
      if (b.type === 'garrison') btns.push(`<button class="btn" id="troopbtn">${b.troop === 'scout' ? '→ Bows' : '→ Scouts'}</button>`);
      ...
      wire = () => {
        const ftb = $('feedtorchbtn'); if (ftb) ftb.onclick = () => feedBrazier(b);
        const trb = $('troopbtn'); if (trb) trb.onclick = () => { b.troop = b.troop === 'scout' ? 'archer' : 'scout'; for (const v of G.villagers) if (v.job === b.id) v.hp = null; uiDirty = true; };
    ...
    const vrole = roleOf(v);
    const job = v.injured > G.day ? 'healing' : vrole ? vrole : v.job != null ? (BUILDS[byId(v.job)?.type]?.name || '—') : (v.age < ADULT_AGE ? 'underfoot' : 'no work');
    ...
    inner = `<h3>${v.age < ADULT_AGE ? '👶' : '🧍'} ${v.name} ${v.family}</h3>` +
      ...
      (vrole && vrole !== 'scout' ? `<div class="sdesc">Stands at ${'♥'.repeat(Math.max(1, Math.ceil(v.hp != null ? v.hp : 3)))}${'♡'.repeat(Math.max(0, 3 - Math.ceil(v.hp != null ? v.hp : 3)))} — mends with a bed and a meal.</div>` : '') +
...
function handleTap(mx, my, shiftHeld) {
  ...
  // tapping a brazier feeds it — same gesture as the hearth
  {
    const tb = buildAt(x, y);
    if (tb && tb.type === 'torch' && tb.built && !tb.ruined) {
      feedBrazier(tb);
      selected = { kind: 'build', id: tb.id };
      uiDirty = true;
      return;
    }
  }
...
function drawBuildingArt(b, now) {
  ...
  switch (b.type) {
    ...
    case 'lookout': {
      // a long ladder, a small roof, no flame at all
      ctx.strokeStyle = '#6a6048'; ctx.lineWidth = 1.2;
      ctx.beginPath();
      ctx.moveTo(px + 5.5, py + 15); ctx.lineTo(px + 7, py - 4);
      ctx.moveTo(px + 10.5, py + 15); ctx.lineTo(px + 9, py - 4);
      ctx.stroke();
      ctx.strokeStyle = '#56503c'; ctx.lineWidth = 0.7;
      for (let i = 0; i < 4; i++) { ctx.beginPath(); ctx.moveTo(px + 5.8 + i * 0.3, py + 12 - i * 4); ctx.lineTo(px + 10.2 - i * 0.3, py + 12 - i * 4); ctx.stroke(); }
      ctx.fillStyle = '#5d5444';
      ctx.fillRect(px + 4.5, py - 5.5, 7, 2.4);                       // the platform
      ctx.fillStyle = '#4a4436';
      ctx.beginPath(); ctx.moveTo(px + 3.6, py - 5.5); ctx.lineTo(px + 8, py - 8.5); ctx.lineTo(px + 12.4, py - 5.5); ctx.closePath(); ctx.fill();
      break;
    }
    case 'muster': {
      // a trampled green, a rack of clubs, a ring of practice stakes
      ctx.fillStyle = 'rgba(138,112,80,0.45)';
      ctx.beginPath(); ctx.ellipse(px + 8, py + 9, 6, 4, 0, 0, 7); ctx.fill();
      ctx.strokeStyle = '#7a6048'; ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(px + 3, py + 13); ctx.lineTo(px + 3, py + 6);
      ctx.moveTo(px + 13, py + 13); ctx.lineTo(px + 13, py + 6);
      ctx.moveTo(px + 2, py + 6.5); ctx.lineTo(px + 14, py + 6.5);
      ctx.stroke();
      ctx.strokeStyle = '#cabb9d'; ctx.lineWidth = 0.9;
      for (let i = 0; i < 3; i++) { ctx.beginPath(); ctx.moveTo(px + 4.5 + i * 3.4, py + 6.5); ctx.lineTo(px + 5.5 + i * 3.4, py + 11); ctx.stroke(); }
      break;
    }
    case 'garrison': {
      // squared timber and stone, arrow slits, a quiver by the door
      ctx.fillStyle = ['#8a5050', '#824a4e', '#915454'][h % 3];
      ctx.fillRect(px + 2, py + 4, 12, 11);
      ctx.fillStyle = '#5a5d66';
      ctx.fillRect(px + 1, py + 12, 14, 3);                           // stone footing
      ctx.fillStyle = '#2c2418';
      ctx.fillRect(px + 6.8, py + 9, 2.4, 6);
      ctx.fillStyle = '#1c1c22';
      ctx.fillRect(px + 4, py + 6, 1, 3); ctx.fillRect(px + 11, py + 6, 1, 3);   // slits
      ctx.strokeStyle = '#cabb9d'; ctx.lineWidth = 0.7;
      ctx.beginPath(); ctx.moveTo(px + 12.6, py + 12); ctx.lineTo(px + 14.2, py + 7); ctx.stroke();
      break;
    }
    default: {
...
function drawVillager(v, now) {
  ...
  {
    const role2 = roleOf(v);
    if (role2 === 'soldier') {
      ctx.strokeStyle = '#e8e0d0'; ctx.lineWidth = 0.8;
      ctx.beginPath(); ctx.moveTo(bx + 1.5, by + 2); ctx.lineTo(bx + 3.5, by - 4.5); ctx.stroke();
    } else if (role2 === 'militia') {
      ctx.strokeStyle = '#a8845a'; ctx.lineWidth = 1.1;
      ctx.beginPath(); ctx.moveTo(bx + 1.6, by + 1); ctx.lineTo(bx + 3, by - 2.6); ctx.stroke();
      ctx.fillStyle = '#a8845a';
      ctx.beginPath(); ctx.arc(bx + 3.1, by - 2.9, 0.9, 0, 7); ctx.fill();      // a club, honest and ugly
    } else if (role2 === 'archer') {
      ctx.strokeStyle = '#cabb9d'; ctx.lineWidth = 0.8;
      const dr = v.state === 'fight' ? 0.4 : 0;
      ctx.beginPath(); ctx.arc(bx + r * 0.9, by - r * 0.3, 2.6, -1.2 - dr, 1.2 + dr); ctx.stroke();
    } else if (role2 === 'scout') {
      // a captive spark on a short pole — the smallest hearth there is
      ctx.strokeStyle = '#54401f'; ctx.lineWidth = 0.8;
      ctx.beginPath(); ctx.moveTo(bx + 1.6, by + 1); ctx.lineTo(bx + 2.8, by - 3.4); ctx.stroke();
      const fl2 = 1 + Math.sin(now / 100 + v.id) * 0.3;
      ctx.fillStyle = 'rgba(255,190,90,0.5)';
      ctx.beginPath(); ctx.arc(bx + 2.9, by - 4, 2.6 * fl2, 0, 7); ctx.fill();
      ctx.fillStyle = 'rgba(255,230,150,0.95)';
      ctx.beginPath(); ctx.arc(bx + 2.9, by - 4, 1 * fl2, 0, 7); ctx.fill();
    }
  }
...
function draw() {
  ...
  // lookouts and scouts grant grey, lightless sight
  const visSrcs = visionSources();

  for (let y = y0; y < y1; y++) for (let x = x0; x < x1; x++) {
    const tileLit = litDraw(x, y);
    const tileScouted = !tileLit && visSrcs.length && isScouted(x + 0.5, y + 0.5, visSrcs);
    if (!tileLit && !tileScouted) continue;
    const t = tile(x, y);
    ...
    // scouted ground: seen, not held — grey and unprotected
    if (tileScouted) {
  ...
  for (const b of G.builds) {
    ...
    const px = b.x * TILE, py = b.y * TILE;
    if (b.ruined) {
      // a smoulder of beams — for a few days, anyway
      const cool = clamp((G.day - b.ruinedAt) / 4, 0, 1);
      ctx.fillStyle = '#26221e';
      ctx.beginPath(); ctx.ellipse(px + 8, py + 10, 6.5, 4, 0, 0, 7); ctx.fill();
      ctx.fillStyle = '#3a322a';
      ctx.fillRect(px + 3, py + 7, 5, 2); ctx.fillRect(px + 8, py + 9, 6, 1.8);
      ctx.save(); ctx.translate(px + 11, py + 6); ctx.rotate(0.6); ctx.fillRect(0, 0, 5, 1.6); ctx.restore();
      if (cool < 0.8) {
        for (let i = 0; i < 3; i++) {
          const eh = tHash(b.x + i, b.y);
          ctx.fillStyle = `rgba(255,${120 + (eh % 60)},50,${(1 - cool) * (0.4 + 0.3 * Math.sin(now / 300 + i * 2))})`;
          ctx.fillRect(px + 4 + (eh % 8), py + 8 + ((eh >> 3) % 4), 1.4, 1.4);
        }
        if (Math.random() < 0.02) smokes.push({ x: b.x + 0.3 + Math.random() * 0.4, y: b.y + 0.4, life: 0.8 });
      }
    ...
    if (!b.built) {
      ...
      const tf = b.fuel || 0;
      if (tf > 0) {
        const fl = (0.5 + 0.5 * Math.min(1, tf / 24)) * (1 + Math.sin(now / 110 + b.x) * 0.25);
        ctx.fillStyle = 'rgba(255,180,80,0.9)';
        ctx.beginPath(); ctx.arc(px + 8.2, py + 4, 2.6 * fl, 0, 7); ctx.fill();
        ctx.fillStyle = 'rgba(255,230,150,0.9)';
        ctx.beginPath(); ctx.arc(px + 8.2, py + 4, 1.2 * fl, 0, 7); ctx.fill();
      } else {
        // cold and dark — a dead brazier is a promise broken
        ctx.fillStyle = '#3a3a40';
        ctx.beginPath(); ctx.arc(px + 8.2, py + 4.5, 1.8, 0, 7); ctx.fill();
        ctx.fillStyle = 'rgba(120,120,130,0.5)';
        ctx.fillRect(px + 7.4, py + 3.4, 1.6, 0.8);
      }
    } else {
  ...
  for (const m of G.monsters) {
    // visible in light or under a lookout's gaze — never in the blind dark
    if (!litDraw(Math.floor(m.x), Math.floor(m.y)) && !(visSrcs.length && isScouted(m.x, m.y, visSrcs))) continue;
    const px = m.x * TILE, py = m.y * TILE;
    const def = MONSTER_TYPES[m.type] || MONSTER_TYPES.skitter;
    const wob = Math.sin(now / (m.type === 'hulk' ? 500 : m.type === 'breaker' ? 160 : 250) + m.x * 5) * 0.8;
    const sz = m.type === 'hulk' ? [7.5, 5.2] : m.type === 'breaker' ? [5, 3.4] : [4, 2.8];
    ctx.fillStyle = m.type === 'hulk' ? 'rgba(28,18,34,0.92)' : m.type === 'breaker' ? 'rgba(35,15,30,0.85)' : 'rgba(20,12,28,0.8)';
    ctx.beginPath(); ctx.ellipse(px + wob * 0.3, py, sz[0] + wob * 0.4, sz[1], 0, 0, 7); ctx.fill();
    if (m.type === 'breaker') {
      // spined, like something built for breaking
      ctx.strokeStyle = 'rgba(60,30,50,0.9)'; ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(px - 2, py - 3); ctx.lineTo(px - 2.6, py - 5);
      ctx.moveTo(px + 1, py - 3.2); ctx.lineTo(px + 1.4, py - 5.4);
      ctx.stroke();
    }
    if (m.type === 'hulk') {
      ctx.fillStyle = 'rgba(28,18,34,0.92)';
      ctx.beginPath(); ctx.arc(px + sz[0] * 0.7, py - sz[1] * 0.6, 2.6, 0, 7); ctx.fill();   // a head, of sorts
    }
    if (Math.sin(now / 300 + m.x * 7) > -0.7) {
      ctx.fillStyle = m.type === 'hulk' ? '#ff3018' : m.type === 'breaker' ? '#ff5a30' : '#e04040';
      const ey = m.type === 'hulk' ? py - 3.2 : py - 1.5;
      ctx.fillRect(px - 2.2, ey, 1.5, 1.5);
      ctx.fillRect(px + 0.8, ey, 1.5, 1.5);
    }
    // the light eats at them, visibly
    if (m.burning) {
      ctx.strokeStyle = `rgba(255,220,140,${0.35 + 0.25 * Math.sin(now / 70)})`;
      ctx.lineWidth = 1;
      ctx.beginPath(); ctx.ellipse(px, py, sz[0] + 1.5, sz[1] + 1.2, 0, 0, 7); ctx.stroke();
    }
    if (m.hitFlash > 0) {
      ctx.fillStyle = `rgba(255,255,255,${m.hitFlash * 4})`;
      ctx.beginPath(); ctx.ellipse(px, py, sz[0], sz[1], 0, 0, 7); ctx.fill();
    }
    // wounds, shown plainly
    if (m.hp < def.hp) {
      for (let i = 0; i < def.hp; i++) {
        ctx.fillStyle = i < Math.ceil(m.hp) ? '#e04040' : 'rgba(80,40,40,0.6)';
        ctx.fillRect(px - def.hp + i * 2.2, py - sz[1] - 4, 1.6, 1.6);
      }
    }
    // what it stole, slung under it
    if (m.loot) {
      ctx.fillStyle = m.loot.kind === 'food' ? '#c2a85a' : '#8a6a3a';
      ctx.fillRect(px - 1.4, py + sz[1] - 1, 3, 2.2);
    }
  }
  // arrows and blows
  for (const f of FX) {
    if (f.kind === 'arrow') {
      const t2 = Math.min(1, f.t);
      ctx.strokeStyle = `rgba(220,210,180,${1 - Math.max(0, f.t - 1) * 2.5})`;
      ctx.lineWidth = 0.6;
      const ax = lerp(f.x1, f.x2, t2) * TILE, ay = lerp(f.y1, f.y2, t2) * TILE;
      ctx.beginPath(); ctx.moveTo(ax - (f.x2 - f.x1) * 2, ay - (f.y2 - f.y1) * 2); ctx.lineTo(ax, ay); ctx.stroke();
    } else if (f.kind === 'hit') {
      ctx.strokeStyle = `rgba(255,200,120,${1 - f.t / 1.4})`;
      ctx.lineWidth = 1.2;
      ctx.beginPath(); ctx.arc(f.x * TILE, f.y * TILE, 2 + f.t * 5, 0, 7); ctx.stroke();
    }
  }
